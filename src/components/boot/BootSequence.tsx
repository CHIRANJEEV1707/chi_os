...
export default function BootSequence({ onComplete, onSkip }: BootSequenceProps) {
  ...
  const handleBootCompleteWithAchievement = () => {
    unlock('first_boot');
    onComplete();
  };
  ...
  useEffect(() => {
    ...
    sequenceTimers.push(setTimeout(handleBootCompleteWithAchievement, 7500)); // Desktop Reveal (1500)
